export default function TimelineBar({ days = [] }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-4">
      ...
      <p className="text-xs text-gray-500 mb-3">Green = income, red = expenses. Darker = higher amount.</p>
          ...
          {days.map((day) => {
            ...
            return (
              <div key={day.date} className="flex flex-col items-center gap-1">
                <div
                  title={`${day.date} · ${hasIncome ? 'income' : 'expenses'} ${value.toFixed(0)} · balance ${Number(day.running_balance ?? day.runningBalance ?? 0).toFixed(0)}`}
